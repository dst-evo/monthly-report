\section{Produzierte Boxen}
\begin{frame}
    \frametitle{Produzierte Pakete pro Tag}
    \begin{figure}
        \centering
        \includegraphics[width=0.75\pagewidth]{single_linechart_pb_tot}
    \end{figure}
\end{frame}

\begin{frame}
    \frametitle{Produzierte Pakete pro Tag und Maschine}

    \begin{figure}
        \centering
        \includegraphics[width=0.75\pagewidth]{daily_barplot_pb}
    \end{figure}

\end{frame}

\begin{frame}
    \frametitle{Total produzierte Boxen}

    \begin{figure}
        \centering
        \includegraphics[width=0.75\pagewidth]{donut_chart_total_box_pm}
    \end{figure}

\end{frame}

% TODO: lower graphic scaling to match frame size
\begin{frame}
    \frametitle{Total produzierte Pakete pro Monat}

    \begin{figure}
        \centering
        \includegraphics[width=0.75\pagewidth ]{historic_single_bar_total}
    \end{figure}

\end{frame}